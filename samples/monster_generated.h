...
template <>
struct EquipmentTraits<MyGame::Sample::Weapon> {
  static const Equipment enum_value = Equipment_Weapon;
};

template <typename T>
struct EquipmentUnionTraits {
  static const Equipment enum_value = Equipment_NONE;
};

template <>
struct EquipmentUnionTraits<MyGame::Sample::WeaponT> {
  static const Equipment enum_value = Equipment_Weapon;
};
...
inline bool operator==(const MonsterT& lhs, const MonsterT& rhs) {
  return ((lhs.pos == rhs.pos) ||
         ...
         (lhs.weapons.size() == rhs.weapons.size() &&
          std::equal(lhs.weapons.cbegin(), lhs.weapons.cend(),
                     rhs.weapons.cbegin(),
                     [](std::unique_ptr<MyGame::Sample::WeaponT> const& a,
                        std::unique_ptr<MyGame::Sample::WeaponT> const& b) {
                       return (a == b) || (a && b && *a == *b);
                     })) &&
...
inline void Monster::UnPackTo(
  ...
  (void)_resolver;
  {
    ...
    if (_e)
      _o->pos =
          std::unique_ptr<MyGame::Sample::Vec3>(new MyGame::Sample::Vec3(*_e));
  }
  {
    auto _e = mana();
    _o->mana = _e;
    ...
    if (_e) {
      ...
      for (::flatbuffers::uoffset_t _i = 0; _i < _e->size(); _i++) {
        if (_o->weapons[_i]) {
          ...
          _o->weapons[_i] = std::unique_ptr<MyGame::Sample::WeaponT>(
...
inline MyGame::Sample::Monster* GetMutableSizePrefixedMonster(void* buf) {
  return ::flatbuffers::GetMutableSizePrefixedRoot<MyGame::Sample::Monster>(
      buf);
}
...
inline std::unique_ptr<MyGame::Sample::MonsterT> UnPackMonster(
  ...
  return std::unique_ptr<MyGame::Sample::MonsterT>(
